...
void main(void)
{
	...
	{// O arquivo ainda não existe
		do // pedindo quantidade de andares
		{
			cout << "Digite a quantidade de andares entre " << QUANTIDADE_MINIMA_ANDARES << " e " << QUANTIDADE_MAXIMA_ANDARES << endl;
		...
		do // pedindo quantidade de quartos
		{
			cout << "Digite a quantidade de quartos entre " << QUANTIDADE_MINIMA_QUARTOS << " e " << QUANTIDADE_MAXIMA_QUARTOS << endl;
	...
	if ((fopen_s(&fdQuantidades, CAMINHO_QUANTIDADES, "wb")) != 0)
	...
	while(true)								// loop infinito
	{
		...
		switch(cOpcaoOper)					// avaliar a opção que foi escolhida pelo operador
		{
			case FAZER_CHECK_IN:
				...
				if (LerQuarto(nAndar, nQuarto, stQuantidades.nQtdeQuartos, fdQuartos, &stQuarto))
				{
			...
			case LANCAR_SERVICO:
				...
				// 2 - Ler os dados do quarto para stQuarto 
				//		Se houver erro, avisar o operador e voltar ao menu
