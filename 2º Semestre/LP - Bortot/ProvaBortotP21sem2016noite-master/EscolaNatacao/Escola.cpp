// Programa Escola de Natação - Prova N2 - 01/06/2016 - Turma da Noite
//
#include "Escola.h"
...
void main(void)
{
	...
	stQtRaias.nQtdeRaias = 0;
	double dValorMensalidade;					// para calcular a mensalidade
	char cOpcao,								// opcao do operador
		cNomeAluno[] = "Raul Victor Coan",		// para conter o nome do aluno que está fazendo a prova
	...
	if (fopen_s(&fdQuantidade, CAMINHO_QUANTIDADE, "r+b") != 0)
	{
		...
		do
		{
			cout << "Informe a quantidade de raias na piscina entre " << QTDE_MINIMA_RAIAS_PISCINA << 
				" e " << QTDE_MAXIMA_RAIAS_PISCINA << ": ";
			cin >> nQtdeTotalDeRaias;

		} while (nQtdeTotalDeRaias < QTDE_MINIMA_RAIAS_PISCINA ||
			nQtdeTotalDeRaias > QTDE_MAXIMA_RAIAS_PISCINA);
		stQtRaias.nQtdeRaias = nQtdeTotalDeRaias;

		if (fwrite(&stQtRaias, sizeof(QTDE_RAIAS), 1, fdQuantidade) == 0)
		{
			cout << "erro ao gravar no arquivo de quantidades!" << endl;
			PAUSA;
			return;
		}
		if (fopen_s(&fdRaia, CAMINHO_RAIA, "w+b") != 0)
		{
			cout << "Erro ao criar arquivo de raias!" << endl;
			PAUSA;
			return;
		}
		for (int i = 1; i <= QTDE_PERIODOS; i++)
		{
			for (int j = 1; j <= stQtRaias.nQtdeRaias; j++)
			{
				...
				{
					cout << "Falha ao gravar raia " << j << " do periodo " << i << endl;
					PAUSA;
					return;
				}
			}
	...
	if (fread_s(&stQtRaias, sizeof(QTDE_RAIAS), 1, 1, fdQuantidade) == 0)
	{
		cout << "erro ao ler arquivo de quantidades!" << endl;
		PAUSA;
		return;
	}
	fclose(fdQuantidade);

	if (fopen_s(&fdRaia, CAMINHO_RAIA, "r+b") != 0)
	{
		cout << "Erro ao abrir arquivo de raias!" << endl;
		PAUSA;
		return;
	}
	...
	while(true)
	{
		...
		switch(cOpcao)								// avalia a opção escolhida
		{
			...
			case CANCELAR_MATRICULA:				
				...
				if (PedirPeriodoRaia(&nPeriodo, &nRaia, 3, stQtRaias.nQtdeRaias, "Excluir aluno") == 0)					// cancelou a ação?
					break;
				nPosicionalRaia = (((nPeriodo - 1)* stQtRaias.nQtdeRaias) + (nRaia - 1)) * sizeof(RAIA);
...
int BuscarRaiaLivreNoPeriodo(int nPeriodo, int nQtRaiasPiscina, RAIA *ptrStRaia, FILE **fdRaia)
{
	...
	for (int i = 1; i <= nQtRaiasPiscina; i++)
	{
		...
		if(fseek(*(fdRaia),  nPosicional * sizeof(RAIA), SEEK_SET) != 0)	//erro de perocura?
		{
			cout << "erro ao procurar local da raia no arquivo." << endl;
			PAUSA;
			return 0;
		}
		if (fread_s(&stRaia, sizeof(RAIA), sizeof(RAIA), 1, *fdRaia) == 0) // erro de leitura?
		{
			cout << "Erro ao ler raia no arquivo." << endl;
			PAUSA;
			return 0;
		}
...
int VerificaParentes(char *ptrSobrenome, FILE **fdRaia)
{
	...
	{
		if (fseek(*(fdRaia),i, SEEK_SET) != 0)	//erro de perocura?
		{
		...
		if (fread_s(&stRaia, sizeof(RAIA), 1, 1, *fdRaia) == 0) // erro de leitura?
		{
			cout << "Erro ao ler raia no arquivo." << endl;
			PAUSA;
			return 0;
...
bool GravaPosicional(int nPosicional, RAIA *ptrStRaia, FILE **fdRaia)
{
	...
	if (fseek(*(fdRaia), nPosicional, SEEK_SET) != 0)	//erro de perocura?
	{
		cout << "erro na procura de posição no arquivo." << endl;
		PAUSA;
		return false;
	}
	if (fwrite(ptrStRaia, sizeof(RAIA), 1, *fdRaia) == 0)	//erro de gravação?
	{
...
bool LerPosicional(int nPosicional, RAIA *ptrStRaia, FILE **fdRaia)
{
	...
	if (fread_s(ptrStRaia, sizeof(RAIA), 1, 1, *fdRaia) == 0)	//erro de leitura?
	{
