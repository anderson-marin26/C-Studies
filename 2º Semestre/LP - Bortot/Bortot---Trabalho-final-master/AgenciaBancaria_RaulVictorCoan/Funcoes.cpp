...
//Função que pede o código de um cliente válido ou zero para cancelar a transação
//Parâmetros:
//	entrada: Char *ptrAcao - ponteiro para um string que contem a ação que está sendo executada
...
int PedirCodigoCliente(char *ptrAcao)
{
	...
	do
	{
		cout << "Digite o código do cliente entre 1 e " << QTDE_CLIENTES << endl
...
//Função que le um cliente de forma posicional (acesso direto)
//		Parâmetros:
...
// 					 - Falso - erro de seek ou de gravação
bool GravarClientePosicional(int nCodigo, CLIENTE *ptrCliente, FILE *fdArquivo)
{
	if (fseek(fdArquivo, (nCodigo - 1) * sizeof(CLIENTE), SEEK_SET) != 0)		//erro
		return false;		//indica que houve erro
	if (fwrite(ptrCliente, sizeof(CLIENTE), 1, fdArquivo) == 0)		//erro de gravação
